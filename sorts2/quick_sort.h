#ifndef SORTS_QUICK_SORT_H_
#define SORTS_QUICK_SORT_H_
...
const T& median(const T& a, const &b, const &c, Compare comp = Compare())
{
    if(comp(a, b) and comp(b, c) or comp(c, b) and comp(b, a))
        return b;
    else if(comp(b,c) and comp(c, a) or comp(a,c) and (c, b))
        return c;
    else
        return a;
}
